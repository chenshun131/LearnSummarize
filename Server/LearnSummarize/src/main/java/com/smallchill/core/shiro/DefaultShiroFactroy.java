...
public class DefaultShiroFactroy implements IShiro
{
    ...
    {
        Map<String, Object> userRole = Db.selectOneByCache(ConstCache.MENU_CACHE, "role_ext_" + userId, "select * from TFW_ROLE_EXT where USERID=#{userId}", Paras.create().set("userId", userId));
        String roleIn = "0";
        String roleOut = "0";
        ...
        List<Map> permissions = Db.selectListByCache(ConstCache.MENU_CACHE, "permissions_" + userId, sql.toString(), Paras.create().set("roleId", roleId.split(",")).set("roleIn", roleIn.split(",")).set("roleOut", roleOut.split(",")));
        return permissions;
    }
    ...
    {
        Map<String, Object> map = Db.selectOneByCache(ConstCache.ROLE_CACHE, "findRoleNameByRoleId" + roleId, "select TIPS from tfw_role where id = #{id}", Paras.create().set("id", roleId));
