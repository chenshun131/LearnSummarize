...
public abstract class BladeValidator extends BladeInterceptor
{
    ...
    {
        if (Func.isAllEmpty(field1, field2))
        {
    ...
    protected void validateSql(String field, String errorMessage)
    {
        ...
        sql = sql.toLowerCase();
        if (sql.indexOf("delete") >= 0 || sql.indexOf("update") >= 0 || sql.indexOf("insert") >= 0 || sql.indexOf("drop") >= 0)
        {
            addError(errorMessage);
        }
    }
    ...
    {
        validateIntegerValue(request.getParameter(field), min, max, errorMessage);
    }
    ...
    protected void validateDate(String field, String min, String max, String errorMessage)
    {
        ...
        try
        {
            validateDate(field, DateKit.parseTime(Func.toStr(min)), DateKit.parseTime(Func.toStr(max)), errorMessage);
        }
        catch (Exception e)
        {
            addError(errorMessage);
    ...
    {
        validateStringValue(request.getParameter(field), minLen, maxLen, errorMessage);
    }
