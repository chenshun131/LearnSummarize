...
public class SideBarTag extends Tag
{
    ...
    public void render()
    {
        try
        {
            ...
            List<Map> sideBar = Db.selectListByCache(MENU_CACHE, "sideBar_" + userId, sql.toString(), Paras.create().set("roleId", roleId.toString().split(",")).set("roleIn", roleIn.split(",")).set("roleOut", roleOut.split(",")));
